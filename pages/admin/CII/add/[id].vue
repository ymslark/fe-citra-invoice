...
const store = async () => {
  try {
    ...
    if (cii.addedInterior.length == 0) throw { message: 'Interior tidak boleh kosong' }

    const response = await cii.storeCII()
    console.log(response)
    alert.showAlertObject({
      message: 'Berhasil Menambah Surat Citra Interior Indonesia',
      type: 'success',
    })
    window.scrollTo(0, 0)
    await cii.setStatusRequest(id)
    setTimeout(() => {
...
<template>
  <div>
    ...
    <VForm ref="refForm" @submit.prevent>
      <VCard class="mt-4">
        <VCardTitle>
          Tambah Surat Citra Interior Indonesia
        </VCardTitle>
        <Tes />
        <VCardItem>
          <VRow>
