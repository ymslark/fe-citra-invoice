      ...
      <VCardTitle>
        <div class="">
          <h2 class="text-lg font-weight-medium d-inline">
            Data Surat Permintaan Faktur 30 Hari Terakhir
          </h2>
        </div>
      </VCardTitle>
      <VCardItem>
...
const filterData = async (page = 1) => {
  try {
    ...
    const query = buildQueryFilterParams({ startDate: start, endDate: end,page, search: search.value, limit:10 }, true);
    const response = await $api.get('/Faktur/filterData', { ...query });
