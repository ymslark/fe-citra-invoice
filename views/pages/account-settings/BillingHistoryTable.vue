...
const deleteInvoice = async id => {
  await $aapi(`/apps/invoice/${id}`, { method: 'DELETE' })
  fetchInvoices()
}
</script>
    ...
    <VCardText class="d-flex align-center flex-wrap gap-4">
      ...
      <div class="d-flex align-end flex-wrap gap-4">
        ...
        <div class="invoice-list-status">
          <AppSelect v-model="selectedStatus" placeholder="Invoice Status" clearable clear-icon="tabler-x"
    ...
    <VDataTableServer v-model="selectedRows" v-model:items-per-page="itemsPerPage" v-model:page="page" show-select
      ...
      <template #item.client="{ item }">
        <div class="d-flex align-center">
          ...
          <div class="d-flex flex-column">
            <NuxtLink class="font-weight-medium text-body-1 text-high-emphasis mb-0 text-link"
      ...
      <template #item.balance="{ item }">
        <VChip v-if="typeof ((resolveInvoiceBalanceVariant(item.balance, item.total)).status) === 'string'"
          :color="resolveInvoiceBalanceVariant(item.balance, item.total).chip.color" size="small" label>
